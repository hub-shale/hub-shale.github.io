...
Before getting started with answering my question, I'll take a quick look at the data to make sure it satisfies some basic requirements for normality. The basic histograms below show that essays have a reasonably normal distribution (with one obvious low outlier). Quizzes grades, on the other hand, look like a log distribution might fit better - unfortunately, the log of quiz grades also looks like it would fit a log normal curve (and so does the log of the log). So at this point I decided to just roll with the original quiz data with the recognition that any standard errors retrieved from this analysis will be effected, but bias should remain unaffected. Likewise, the pronouns data (which will be used in a follow-up analysis) have some clear limitations because of the uneven distribution of students who have listed their pronouns, particularly nonbinary students.

```{r, out.width=c('50%','50%')}
ggplot(data_clean, aes(x=essay)) + geom_histogram(bins = 40)
...
ggplot(data_clean, aes(x=quizzes_pct)) + geom_histogram()

log_quizzes = data_clean %>% 
  mutate(ql = log(quizzes_pct))
ggplot(log_quizzes, aes(x=ql)) + geom_histogram()
...
bias_acf = acf(data_clean$essay, lag.max = 10)
```
...
```{r, out.width='60%', fig.cap="Essay grades are marginally higher for women and lower for nonbinary students as compared to men, but not statistically significant."}
